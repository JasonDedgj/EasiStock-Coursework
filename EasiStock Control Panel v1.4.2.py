...
try: # tries initially to select all records from the table, "Users"
    ...
    values = [("21DoniLa", "Larry", "Doni", "12-12-20", hashed, 2, "jason.dedgjonaj1@gmail.com"), # this is an array of values that /will/ be used with the variable below
              ("15DedgjJa", "Jason", "Dedgj", d2, hashed, 0, "15dedgjonajja@greigcityacademy.co.uk")] 
...
def db_creation():
    ...
    c.execute('''
                    ...
                    [user_created] BLOB NOT NULL,
                    [last_change_user] BLOB,
                    FOREIGN KEY (last_change_user)
    ...
    values = [(None, "Root Record", "255", "£5.00", "Root Record Description Test, very good product!", d1, d2, d3, '21DoniLa', '21DoniLa'), # this is an array of values that /will/ be used with the variable below
            (None, "Root Record2", "255", "£5.00", "Root Record Description Test, very bad product!", d1, d2, d3, '21DoniLa', '21DoniLa')]
...
def user_reg_db(first_name, last_name, date, email):
    ...
    if confirm_user == "2":
        ...
        print(hashed)
        try:
            values = [(Username, first_name.capitalize(), last_name.capitalize(), d2, hashed, 0, email)]
            ...
            exception(first_name, last_name, d2, number, email, hashed)
...
def exception (first_name, last_name, date, number, email, hashed):
    ...
    try:
        ...
        number = number + 1
        exception(first_name, last_name, date, number, email, hashed)
