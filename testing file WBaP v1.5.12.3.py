...
MainMenu.title("EasiStock v1.5.12.3")
...
def Disclaimer(index, win):
        ...
        version = tk.Label ((index), text = "ES v1.5.12.3 DEV BUILD", font = ("Helvetica", 9))
...
def Main(page_place, after_del):
   ...
   ### print(user_e)
    ...
    refresh = Button(MainMenu, text = "⟳")
    refresh.place(x = 10, y = 10 )
...
# ========================================================== > \/ ABP Pre-Req Functions and Variables
    ...
    def edit_check(k, desc, stock, price, name): # =========================================================== Product Editing Input Checks
        ...
        er01 = "Invalid stock input, please enter a number, not a letter/s."
        
        er02 = "Invalid price format, cannot use special characters (!'@:%^&*)"
        er03 = "Invalid price format, cannot use letters (ABC)"
        er04 = "Invalid price format, please input in the following format. 12.99 (£12.99) or 4.2 (£4.20)"
        er05 = "Inavlid price format, cannot use letters (ABC) or special characters (!'@:%^&*)"
        
        er07 =  "Invalid Description input, input is too long (Maximum = 500 Characters) \nYour character count = ", len(desc)

        er08 = "Invalid stock input, cannot use letters (ABC)"
        er09 = "Invalid stock input, cannot use special characters (!'@:%^&*)"
        er10 = "Inavlid stock input, cannot use letters (ABC) or special characters (!'@:%^&*)"

        er11 = "Invalid name input, cannot use previous name."
        ...
        if len(desc) ==0 or len(price) ==0 or len(stock) ==0 or len(name) == 0:
            ...
            if errors == 0:
                ...
                if confirm == True:
                    ...
                    #print("name:", name, "\nstock:", stock, "\nprice:", price, "\nday", d2, "and time", d3)
                    c.execute('''UPDATE Products SET product_name = ?, product_count = ?, product_price = ?, product_desc = ?, last_change_day = ?, last_change_time = ? WHERE product_id = ?''', (name, stock, price_in, desc, d2, d3, k))
                    conn.commit()
                    loaded = False
